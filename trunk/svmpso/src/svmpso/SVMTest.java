...
public class SVMTest {
	...
	public svm_problem getdata(List<Particle> chroms){
		svm_problem prob = new svm_problem();
		prob.l = chroms.size();
		double[] y = new double[prob.l];
		int lenth = chroms.get(0).getDimention();
		svm_node[][] x = new svm_node[prob.l][lenth];
		for(int i=0;i<=prob.l-1;i++){
			y[i] = chroms.get(i).getFitness();
			for(int j = 1;j<=lenth;j++){
				x[i][j-1] = new svm_node();
				x[i][j-1].index = j;
				x[i][j-1].value = (Double)(chroms.get(i).getPosition()[j-1]);
			}
		}
		prob.x = x;
		prob.y = y;
...
public svm_parameter initsvm(double gamma, double c){
	...
	public double predict(svm_model model, Particle chrom){
		double pre_y = 0.0;
		int lenth = chrom.getDimention();
		svm_node[] chromnode = new  svm_node[lenth];		
		for(int i=1;i<=lenth;i++){
			chromnode[i-1] = new svm_node();
			chromnode[i-1].index = i;
			chromnode[i-1].value = (Double)(chrom.getPosition()[i-1]);			
		}
		pre_y = svm.svm_predict(model,chromnode);
		return pre_y;
	}
	
	public void predict(svm_model model, List<Particle> chroms){
		for(Particle chrom:chroms){
			chrom.setFitness(predict(model, chrom), false);
		}
	}
	
	public svm_model getmodel(List<Particle> chroms, double gamma, double c){
