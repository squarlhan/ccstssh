	...
	for(int i = 0; i<=m-1; i++){
		...
		for(int j = 0; j<=n-1; j++){
	    	...
	    	if(j<n-1){
	    		...
	    		y[j+1]= y(mydata1);
	    		total+=((y[j]-1)*(y[j]-1)*(1+10*Math.sin(Math.PI*y[j+1])*Math.sin(Math.PI*y[j+1])));
	    	}
	    	total1+=u(mydata,10,100,4);
		}
		results[i] = Math.PI/n*(10*Math.sin(Math.PI*y[0])*Math.sin(Math.PI*y[0])+total+(y[n-1]-1)*(y[n-1]-1))+total1;
